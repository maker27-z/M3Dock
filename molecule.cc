...
double Molecule_DLIGAND2::score(){
...
//        exit(0);
	...
	for(int i=0; i<natm; i++){
		...
		for(int j=natm - lig_num; j<natm; j++){
			if(rseq[i] == rseq[j]) continue; //protein和ligand不会在同一个残基上
...
void Molecule_DLIGAND2::rdmol2(string fn){
	string ligname = fn;
...
//		if(an=="F" || an=="Cl" || an=="Met") continue;
		...
		atoms.push_back( Atom_DLIGAND2(natm, matype_pro+it, s1) );
        Atom_DLIGAND2 *ap = &atoms.back();
		ap -> setx(str+15);
