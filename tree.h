...
//            if (std::isnan(tmp.second[2]) || std::isinf(tmp.second[2])) {
//                fl a = 1.0/0;
//                fl c = a+1;
//                std::cerr << "Invalid value detected: " << tmp.second[2] << std::endl;
//            }
            assert(!std::isnan(tmp.second[2]) && !std::isinf(tmp.second[2]));
		}
