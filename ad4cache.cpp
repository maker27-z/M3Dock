...
fl ad4cache::eval_deriv_ad4(model& m, fl v) const { // sets m.minus_forces
    fl e = 0;
    sz nat = num_atom_types(atom_type::AD);
...
fl ad4cache::eval_deriv_DLIGAND2(const output_type_MODEA& outputTypeModea, fl v) const{
    return 0;
}
	...
	while(std::getline(in, line)) {
		...
		if (line_counter == 5) {
			...
			VINA_FOR(i, 3) {
				...
				// (number of sampled points == n + 1)
//                gd[i].n = sz(std::ceil(std::atof(fields[i + 1].c_str()) / spacing));
                gd[i].n = std::atoi(fields[i + 1].c_str());
                if (gd[i].n % 2 == 1) {
...
fl ad4cache::eval_inter_ligand_receptor_deriv(      model& m, fl v) const{

}
fl ad4cache::eval_intra_deriv(      model& m, fl v) const{
