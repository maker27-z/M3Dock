...
struct model {
    ...
    fl ad4_eval_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g);
    fl vdw_eval_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g);
    fl eval_inter_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g);
    fl eval_intramolecular_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g);
	fl eval_intramolecular(                            const precalculate& p,                  const vec& v, const conf& c);
	fl eval_adjusted      (const scoring_function& sf, const precalculate& p, const igrid& ig, const vec& v, const conf& c, fl intramolecular_energy,change &g);
    fl eval_inter(const precalculate& p, const vec& v) const;//AD4
    fl evalo(const precalculate& p, const vec& v) const;//AD4
    fl evalig(const precalculate& p, const vec& v) const;//AD4
    void set_m_atom_typing_used(){ m_atom_typing_used = atom_type::AD;}//AD4
    ...
    atomv& get_protein_atoms(){return grid_atoms;}
    atomv& get_ligand_atoms(){return atoms;}
    sz get_m_num_movable_atoms(){return m_num_movable_atoms;}
    atomv atoms; // movable, inflex
    vecv coords;
    vecv minus_forces;
    model() : m_num_movable_atoms(0), m_atom_typing_used(atom_type::XS) {};
    vector_mutable<ligand> get_m_ligands(){return ligands;}
private:
