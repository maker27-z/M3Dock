...
public:
    ...
    fl eval_deriv_ad4(      model& m, fl v) const; // needs m.coords, sets m.minus_forces // clean up
    fl eval_deriv_DLIGAND2(const output_type_MODEA& outputTypeModea, fl v) const; // needs m.coords, sets m.minus_forces // clean up
    ...
    fl eval_inter_ligand_receptor_deriv(      model& m, fl v) const;
    fl eval_intra_deriv(      model& m, fl v) const;
private:
	grid_dims m_gd;
