		...
		VINA_FOR_IN(j, m.atoms) {
			...
			if(t1 < n && t2 < n) {
                ...
                if (is_atom_in_ligand(i) || is_atom_in_ligand(j)) {
                    inter_pairs.push_back(interacting_pair(type_pair_index, i, j));
                }
			}
		}
	...
	VINA_FOR_IN(i, atoms) {
		...
		VINA_RANGE(j, i+1, atoms.size()) {
			...
			if(mobility(i, j) == DISTANCE_VARIABLE && !has(bonded_atoms, j)) {
				...
				sz t1 = atoms[i].get  ( atom_type::AD);
				sz t2 = atoms[j].get  ( atom_type::AD);
				sz n  = num_atom_types( atom_type::AD);
...
fl model::eval_deriv  (const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g) { // clean up
    ...
    //根据网格计算蛋白与配体之间的能量
	fl e = ig.eval_deriv(*this, v[1]);
    //计算other_pairs原子对之间的能量
...
//ad4_eval_deriv
fl model::ad4_eval_deriv(const precalculate& p, const igrid& ig, const vec& v,  const conf& c,change& g) { // clean up
    // INTER ligand - grid
    fl e = ig.eval_deriv_ad4(*this, v[1]); // sets minus_forces, except inflex
...
//vdw_eval_deriv
fl model::vdw_eval_deriv(const precalculate& p, const igrid& ig, const vec& v,  const conf& c,change& g) { // clean up
//    // INTER ligand - grid
...
fl model::eval_inter_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g){
    set(c);
    fl all_grids = 0;
    fl flex_grids = 0;
    fl lig_grids = 0;
    ...
    //all_grids: ligand & flex -- grid
    all_grids = ig.eval_deriv(*this, v[1]); // [1] ligand & flex -- grid
    flex_grids = ig.eval_intra_deriv(*this, v[1]);
    lig_grids = all_grids - flex_grids;
//    fl a = ig.eval_inter_ligand_receptor_deriv(*this, v[1]);
    e += lig_grids;
//    e += ig.eval_inter_ligand_receptor_deriv(*this, v[1]);


    //inter_pairs: ligand - flex
    if (!inter_pairs.empty()){
        e += eval_interacting_pairs_deriv(p, v[2], inter_pairs, coords, minus_forces); // adds to minus_forces
    }


//    VINA_FOR_IN(i,minus_forces){
//        minus_forces[i] *= 0.8;
//    }
    ligands.derivative(coords, minus_forces, g.ligands);
    flex   .derivative(coords, minus_forces, g.flex); // inflex forces are ignored
...
fl model::eval_intramolecular_deriv(const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g){
    set(c);
    fl e = 0;

    e += ig.eval_intra_deriv(*this, v[1]);

    // internal for each ligand
    VINA_FOR_IN(i, ligands)
        e += eval_interacting_pairs_deriv(p, v[0], ligands[i].pairs, coords,minus_forces); // coords instead of internal coords

    //flex_i -- flex_i and flex_i -- flex_j
    e += eval_interacting_pairs_deriv(p, v[2], other_pairs, coords,minus_forces);
...
fl model::eval_intramolecular(const precalculate& p, const vec& v, const conf& c) {
    //set(c)需要现根据当前个体的所有原子坐标与model中的所有原子坐标对齐，得到真实m变量中的coords
	set(c);
	fl e = 0;
...
//AD4
fl model::eval_inter(const precalculate& p, const vec& v) const { // clean up
    fl e = eval_interacting_pairs(p, v[2], inter_pairs, coords);
    return e;
}
fl model::evalo(const precalculate& p, const vec& v) const { // clean up
    fl e = eval_interacting_pairs(p, v[2], other_pairs, coords);
    return e;
}
fl model::evalig(const precalculate& p, const vec& v) const { // clean up
    fl e = 0;
    VINA_FOR_IN(i, ligands)
        e += eval_interacting_pairs(p, v[0], ligands[i].pairs, coords); // probably might was well use coords here
    return e;
...
fl model::eval_adjusted      (const scoring_function& sf, const precalculate& p, const igrid& ig, const vec& v, const conf& c, fl intramolecular_energy,change &g) {
    fl e = eval(p, ig, v, c); // sets c
    return sf.conf_independent(*this, e - intramolecular_energy);
//    fl e = eval_deriv(p, ig, v, c,g);
//    return e - intramolecular_energy;
}
