/*
coords
   Copyright (c) 2006-2010, The Scripps Research Institute

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.

   Author: Dr. Oleg Trott <ot14@columbia.edu>, 
           The Olson Lab, 
           The Scripps Research Institute

*/

#include "coords.h"
bool sort_comparator(const output_type_nsga2& x,const output_type_nsga2& y);
bool sort_comparator_ptr(const output_type_nsga2* x, const output_type_nsga2* y);
fl rmsd_upper_bound(const vecv& a, const vecv& b) {
	VINA_CHECK(a.size() == b.size());
	fl acc = 0;
	VINA_FOR_IN(i, a) 
		acc += vec_distance_sqr(a[i], b[i]);
	return (a.size() > 0) ? std::sqrt(acc / a.size()) : 0;
}

std::pair<sz, fl> find_closest(const vecv& a, const output_container& b) {
	std::pair<sz, fl> tmp(b.size(), max_fl);
	VINA_FOR_IN(i, b) {
		fl res = rmsd_upper_bound(a, b[i].coords);
		if(i == 0 || res < tmp.second)
			tmp = std::pair<sz, fl>(i, res);
	}
	return tmp;
}
std::pair<sz, fl> find_closest(const vecv& a, const output_container_nsga2 & b) {
    std::pair<sz, fl> tmp(b.size(), max_fl);
    VINA_FOR_IN(i, b) {
        fl res = rmsd_upper_bound(a, b[i].coords);
        if(i == 0 || res < tmp.second)
            tmp = std::pair<sz, fl>(i, res);
    }
    return tmp;
}
void add_to_output_container(output_container& out, const output_type& t, fl min_rmsd, sz max_size) {
	std::pair<sz, fl> closest_rmsd = find_closest(t.coords, out);
	if(closest_rmsd.first < out.size() && closest_rmsd.second < min_rmsd) { // have a very similar one
		if(t.e < out[closest_rmsd.first].e) { // the new one is better, apparently
			out[closest_rmsd.first] = t; // FIXME? slow
		}
	}
	else { // nothing similar
		if(out.size() < max_size)
			out.push_back(new output_type(t)); // the last one had the worst energy - replacing 
		else
			if(!out.empty() && t.e < out.back().e) // FIXME? - just changed
				out.back() = t; // FIXME? slow
	}
	out.sort();
}


void add_to_output_container(output_container_nsga2 & out,const output_type_nsga2& t, fl min_rmsd, sz max_size) {
//    std::pair<sz, fl> closest_rmsd = find_closest(t.coords, out);
//    if(closest_rmsd.first < out.size() && closest_rmsd.second < min_rmsd) { // have a very similar one
//        if(t.e < out[closest_rmsd.first].e) { // the new one is better, apparently
//            out[closest_rmsd.first] = t; // FIXME? slow
//        }
//    }
//    else { // nothing similar
        if(out.size() < max_size)
            out.push_back(new output_type_nsga2(t)); // the last one had the worst energy - replacing
        else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
            out.back() = t; // FIXME? slow
//        if(!out.empty() && sort_comparator(t , out.back())) // FIXME? - just changed
//            out.back() = t; // FIXME? slow
//    }
//    out.sort();
//    sort(out.begin(),out.end(), sort_comparator);
//	for (int i = 0; i < out.size()-1; i++) {
//		for (int j = 0; j < out.size() - 1 - i; j++) {
//			if (out[j+1].e < out[j].e) {
//				output_type_nsga2 tmp = out[j];
//				out[j] = out[j + 1];
//				out[j + 1] = tmp;
//			}
//		}
//	}
}

std::pair<sz, fl> find_closest_(const vecv& a, const output_container_MODEA & b) {
    std::pair<sz, fl> tmp(b.size(), max_fl);
    VINA_FOR_IN(i, b) {
        fl res = rmsd_upper_bound(a, b[i].coords);
        if(i == 0 || res < tmp.second)
            tmp = std::pair<sz, fl>(i, res);
    }
    return tmp;
}
bool dominated(output_type_MODEA out1, output_type_MODEA out2){
    std::vector<fl> obj_value1 = out1.objectives;
    std::vector<fl> obj_value2 = out2.objectives;
    for(sz i = 0;i<obj_value1.size();i++){
        if(obj_value1[i] > obj_value2[i]){
            return false;
        }
    }
    return true;
}
bool sort_comparator_e(const output_type_MODEA& x,const output_type_MODEA& y){
    return x.e < y.e;
}
void add_to_output_container(output_container_MODEA & out,const output_type_MODEA& t, fl min_rmsd, sz max_size) {
//    if(out.size() < max_size)
        out.push_back(new output_type_MODEA(t)); // the last one had the worst energy - replacing
//    else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
//        out.back() = t; // FIXME? slow

//    std::pair<sz, fl> closest_rmsd = find_closest_(t.coords, out);
//    if(closest_rmsd.first < out.size() && closest_rmsd.second < min_rmsd) { // have a very similar one
//        if(dominated(t,out[closest_rmsd.first]) && t.e < out[closest_rmsd.first].e) { // the new one is better, apparently
//            out[closest_rmsd.first] = t; // FIXME? slow
//        }
//    }
//    else { // nothing similar
//        if(out.size() < max_size)
//            out.push_back(new output_type_MODEA(t)); // the last one had the worst energy - replacing
//        else
//        if(!out.empty() && dominated(t, out.back()) && t.e < out.back().e) // FIXME? - just changed
//            out.back() = t; // FIXME? slow
//    }
//    out.sort(out.begin(),out.end(), sort_comparator_e);
}

void add_to_output_container_(output_container_MODEA & out,const output_type_MODEA& t, fl min_rmsd, sz max_size) {
//    if(out.size() < max_size)
//        out.push_back(new output_type_MODEA(t)); // the last one had the worst energy - replacing
//    else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
//        out.back() = t; // FIXME? slow

//    std::pair<sz, fl> closest_rmsd = find_closest_(t.coords, out);
//    if(closest_rmsd.first < out.size() && closest_rmsd.second < min_rmsd) { // have a very similar one
//        if(t.e < out[closest_rmsd.first].e) { // the new one is better, apparently
//            out[closest_rmsd.first] = t; // FIXME? slow
//        }
//    }
//    else { // nothing similar
//        if(out.size() < max_size)
            out.push_back(new output_type_MODEA(t)); // the last one had the worst energy - replacing
//        else
//            if(!out.empty() && t.e < out.back().e) // FIXME? - just changed
//            out.back() = t; // FIXME? slow
//    }
//    out.sort(out.begin(),out.end(), sort_comparator_e);
}

void add_to_output_container(output_container_SMPSO & out,const output_type_SMPSO& t, fl min_rmsd, sz max_size) {
    if(out.size() < max_size)
        out.push_back(new output_type_SMPSO(t)); // the last one had the worst energy - replacing
    else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
        out.back() = t; // FIXME? slow
}

void add_to_output_container(output_container_MOEAD & out,const output_type_MOEAD& t, fl min_rmsd, sz max_size) {
    if(out.size() < max_size)
        out.push_back(new output_type_MOEAD(t)); // the last one had the worst energy - replacing
    else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
        out.back() = t; // FIXME? slow
}

void add_to_output_container(output_container_MPSOD & out,const output_type_MPSOD& t, fl min_rmsd, sz max_size) {
    if(out.size() < max_size)
        out.push_back(new output_type_MPSOD(t)); // the last one had the worst energy - replacing
    else  if(!out.empty() && t.e < out.back().e) // FIXME? - just changed{
        out.back() = t; // FIXME? slow
}