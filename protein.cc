...
int DEBUG = 0;
string datadir = "";
Protein_DLIGAND2::Protein_DLIGAND2(const string fn){
	reset();
	if(fn == "--"){
		pdbnm = "STDIN";
		rdpdb(stdin); return;
...
void Protein_DLIGAND2::popResidues(int nr){
	if(nr <= 0) return;
	assert(nres >= nr);
	nres -= nr;
	natm = rstart[nres];
...
void Protein_DLIGAND2::rdpdb(FILE *fp){
...
void Protein_DLIGAND2::addAtom(string line){
}
// require to call "initRestypes" first
bool Protein_DLIGAND2::addRes(string rn0){
	char rn[4]; sscanf(rn0.c_str(), "%s", rn);
	int rid = resDefine(rn);
	return addRes(rid);
}
bool Protein_DLIGAND2::addRes(int rid){
	...
	for(int i=0; i<ap.size(); i++){
		...
		int it = ap[i].gettype();
		atoms.push_back(Atom_DLIGAND2(natm, it, an));
		rseq.push_back(nres);
		natm ++;
	}
...
bool Protein_DLIGAND2::initAtom(string line){
...
int Protein_DLIGAND2::findatom(int ir, string an){
...
void Protein_DLIGAND2::wrpdb(string fn){
	FILE *fp = openfile(fn, "w");
	wrpdb(fp); fclose(fp);
}
void Protein_DLIGAND2::wrpdb(FILE *fp){
	char fmt1[] = "ATOM%7d  %-4s%3s %c%4d%12.3f%8.3f%8.3f\n";
	char chn0 = '\n';
	for(int i=0; i<atoms.size(); i++){
        Atom_DLIGAND2 *ap = &atoms.at(i);
...
void Protein_DLIGAND2::calphipsi(int ir, double *ag){
	int i1 = rstart[ir];
	if(ir > 0){
		int i0 = rstart[ir-1];
...
bool Protein_DLIGAND2::iscomplete(){
	bool bcomp=true;
...
void Protein_DLIGAND2::initneib(){
...
void Protein_DLIGAND2::initneib1_sc(int ir){		// specially for mutation
	int id = resid.at(ir);
...
void Protein_DLIGAND2::eraseAtoms(int idx, int na){
...
void Protein_DLIGAND2::insertAtoms(int idx, int na){
	...
	atoms.insert(atoms.begin()+idx, na, Atom_DLIGAND2());
	natm += na;
}
void Protein_DLIGAND2::initAtomint(int i0){
	int ir = rseq[i0];
...
void Protein_DLIGAND2::initAtomzcrd0(int i0){
...
void Protein_DLIGAND2::calAtomzcrd(int ia){
...
void Protein_DLIGAND2::addCB_GLY(int ir){
...
int Protein_DLIGAND2::getReschi(int ir, int *idx){
	string rn = rnam[ir];
	int nchi=0, mchi=4;
...
void Protein_DLIGAND2::resMutation(int ir, string rn){
	int id = resDefine(rn);
	mutAA(ir, id);
}
void Protein_DLIGAND2::mutAA(int ir, int id){
...
int Protein_DLIGAND2::mutRotamer(int ir, int irot){
...
double Protein_DLIGAND2::calResclash(int ir){
	double nclash=0.;
	for(int i=0; i<natm; i++){
...
//
Atom_DLIGAND2::Atom_DLIGAND2(int is, int it, string an){
	aseq=is; type=it; name=an;
	bfill=false;
}
void Atom_DLIGAND2::addneib(int ia){
...
bool Protein_DLIGAND2::makexyz1(int ia){
	double *xa, *xb, *xc;
