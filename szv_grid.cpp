...
szv_grid::szv_grid(const grid_dims& gd, precalculate &p, Molecule_DLIGAND2 mol,fl cutoff_sqr) : m_data(gd[0].n, gd[1].n, gd[2].n) {
    vec end;
    VINA_FOR_IN(i, gd) {
        m_init[i] = gd[i].begin;
        end   [i] = gd[i].end;
...
//    const sz nat = num_atom_types(m.atom_typing_used());
    ...
    szv relevant_indexes;
    VINA_FOR(i, mol.natm-mol.lig_num) {
        const Atom_DLIGAND2& a = mol.atoms[i];
        vec tmp;
        VINA_FOR_IN(j, tmp)
            tmp[j] = a.getx()[j];
        if(a.type < matype_pro && brick_distance_sqr(m_init, end, tmp) < cutoff_sqr)
            relevant_indexes.push_back(i);
    }
        ...
        VINA_FOR(y, m_data.dim1())
            VINA_FOR(z, m_data.dim2()) {
                VINA_FOR_IN(ri, relevant_indexes) {
                    ...
                    VINA_FOR_IN(j, tmp)
                        tmp[j] = a.getx()[j];
                    if(brick_distance_sqr(index_to_coord(x, y, z), index_to_coord(x+1, y+1, z+1), tmp) < cutoff_sqr)
                        m_data(x, y, z).push_back(i);
                }
            }
}
