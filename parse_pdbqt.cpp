...
struct parsed_atom : public atom {
	unsigned number;
    parsed_atom(sz ad_, fl charge_, const vec& coords_, unsigned number_, std::string ad_name_) : number(number_){
        strcpy(ad_name, ad_name_.c_str());
        ad_number = number;
		ad = ad_;
		charge = charge_;
		coords = coords_;
...
parsed_atom parse_pdbqt_atom_string(const std::string& str) {
    ...
    std::string ad_name_string;
    ad_name_string = checked_convert_substring<std::string>(str, 12, 16, "ad_name");
	...
	parsed_atom tmp(ad, charge, coords, number,ad_name_string);
