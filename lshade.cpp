...
//	fl d_torsions = 0;
//	fl torsion_;
//	for (sz i = t_index; i < V_d_torsions.size(); i++) {
...
//}
//
//void armijo(conf& x, const change& g, energy_cal& energy, const fl f0, const change& p, conf& x_new, change& g_new, fl& f1) {
//	const fl c0 = 0.0001;
//	const unsigned max_trial = 12;
//	const fl multiplier = 0.5;
//	fl alpha = 1;
//
//	const fl pg = 1.0;
//
//	for (sz i = 0; i < max_trial; i++) {
//		x_new = x;
//		increment(x_new, p, alpha);
//		f1 = energy(x_new, g_new);
//		if (f1 - f0 < c0 * alpha * pg)
//			break;
//		alpha *= multiplier;
//	}
//}
//fl run_adam(conf& x, change& g, energy_cal& energy, const fl beta1, const fl beta2, vec& V_d_position, vec& S_d_position, vec& V_d_orientation, vec& S_d_orientation, flv& V_d_torsions, flv& S_d_torsions, unsigned& t) {
//	conf x_new(x);  //存放新构象
//	change g_new(g); //存放新构象的导数
//
//	fl f0 = energy(x, g); //计算传入构象的能量，并计算此构象的导数
//	fl orig = f0;   //保存传入的原始构象的能量
//	change g_orig(g);  //保存原始构象对应的导数
//	conf x_orig(x);    //保存原始构象
//
//	change p(g); //d
//	flv f_values;
...
//	for (sz i = 0; i < 18; i++) {
//		t++;
//		eval_d(p, g, beta1, beta2, V_d_position, S_d_position, V_d_orientation, S_d_orientation, t); //计算d,存入p中
//		fl f1 = 0;
//		armijo(x, g, energy, f0, p, x_new, g_new, f1);
...
//		double mean_WL_CR = 0;
//		double mean_WL_F = 0;
//		for (sz i = 0; i < fitness_improvement.size(); i++) {
...
//	lshade_aux aux(m);
//	change g(s);//导数
//	std::ofstream ofs("1sqa_result.txt");
//    //初始化output_type,即最终的输出个体，
//    // 这里传进的是conf_size类型，在构造函数初始化的时候会自动conf(conf_size)，因此就可以把output_type里的conf初始化好了
//	output_type tmp(s, 0);
...
//		aux.nowpopulation[i] = tmp;//保存的并不是构象，而是变化
//		aux.mutatepopulation[i] = tmp;
//	}
//	std::sort(aux.nowpopulation.begin(), aux.nowpopulation.end(), MyCompare);//排序当前种族便于变异
...
//			ofs << "F:" << std::setw(9) << std::setprecision(5) << aux.nowpopulation[j].F << std::endl;
//		}
