...
public:
    Atom_DLIGAND2(){bfill=false; type=-1;}
    Atom_DLIGAND2(int is, int it, string an);
...
// atoms
	...
	vector <Atom_DLIGAND2> atoms;		//natm
public:
	Protein_DLIGAND2(){reset();}
	Protein_DLIGAND2(FILE *fp){reset(); rdpdb(fp);}
	Protein_DLIGAND2(const string fn);
...
//
    ...
    const double *getx(int ia)const{return atoms[ia].getx();}
